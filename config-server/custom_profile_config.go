...
const (
	...
	customFeatureHidePlanSpeed                        = "hide_plan_speed"
...
func readProfileUIColorCustomConfig(yamlContent string) (UIColorCustomConfig, error) {
	...
	if ui != nil {
		...
		if enabledNode := getMapValueNode(ui, "hide_plan_speed"); enabledNode != nil {
			result.HidePlanSpeed = strings.EqualFold(strings.TrimSpace(enabledNode.Value), "true")
		} else if enabledNode := getMapValueNode(ui, "hidePlanSpeed"); enabledNode != nil {
			result.HidePlanSpeed = strings.EqualFold(strings.TrimSpace(enabledNode.Value), "true")
		}
	}
	...
	settings := getMapValueNode(profileRoot, "settings")
	if settings != nil {
		if enabledNode := getMapValueNode(settings, "dns_override_default"); enabledNode != nil {
			result.DNSOverrideDefault = strings.EqualFold(strings.TrimSpace(enabledNode.Value), "true")
		} else if enabledNode := getMapValueNode(settings, "dnsOverrideDefault"); enabledNode != nil {
			result.DNSOverrideDefault = strings.EqualFold(strings.TrimSpace(enabledNode.Value), "true")
		}
	}
...
func writeProfileUIColorCustomConfig(yamlContent string, config UIColorCustomConfig) (string, error) {
	...
	root := ensureDocumentMappingNode(doc)
	profileRootKey := writableProfileConfigRootKey(root)
	profileRoot := ensureMapValueNode(root, profileRootKey)
	...
	if profileRootKey == "nexgen" {
		setMapBoolValue(settings, "dns_override_default", config.DNSOverrideDefault)
		removeMapKeys(settings, "dnsOverrideDefault")
	} else {
...
func (h *Handlers) GetPublicUIColorCustomConfig(w http.ResponseWriter, r *http.Request) {
	...
	jsonResponse(w, map[string]interface{}{
		...
		"hide_plan_speed":                          config.HidePlanSpeed,
		...
		"dns_override_default":                     config.DNSOverrideDefault,
...
func (h *Handlers) SavePublicUIColorCustomConfig(w http.ResponseWriter, r *http.Request) {
	...
	if isUIColorFeatureAllowed(allowedFeatureKeys, customFeatureHidePlanSpeed) {
		targetConfig.HidePlanSpeed = req.HidePlanSpeed
	}
	...
	jsonResponse(w, map[string]interface{}{
		...
		"hide_plan_speed":                          targetConfig.HidePlanSpeed,
		...
		"dns_override_default":                     targetConfig.DNSOverrideDefault,
